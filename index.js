...
function getQuizQuestion(id) {
    ...
    xhttp.open('get',
    'https://opentdb.com/api.php?amount=10&category='+id+'&difficulty=easy&type=multiple',true);
    xhttp.send();
...
function getCatgory() {
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState === 4 && this.status === 200) {
            ...
            onCategoryList(response)
        }else if (this.readyState === 4) {
            //error
        }
    };
    xhttp.open('get',
    'https://opentdb.com/api_category.php',true);
    xhttp.send();

}

function onCategoryList(data){
    var category=''
    data.forEach(function (value,key){
